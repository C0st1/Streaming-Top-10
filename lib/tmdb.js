...
function cleanInFlightMap() {
    ...
    // Note: We can't actually know if a Map value (Promise) is stale without
    // additional tracking. Instead, we set a reasonable size limit.
...
function getTmdbHeaders(apiKey) {
    // If it looks like a bearer token (long hex or has hyphens typical of v4), use Authorization header
    if (apiKey.length > 40 || apiKey.includes('-')) {
...
function appendApiKey(url, apiKey) {
    if (apiKey.length > 40 || apiKey.includes('-')) return url; // Bearer token goes in header
...
function getRpdbPosterUrl(imdbId, rpdbApiKey) {
    ...
    // SEC-11 FIX: Validate RPDB key format to prevent URL injection
...
async function _matchTMDBInternal(item, type, apiKey, overrides, title, year, cacheKey) {
    try {
        ...
        if (overrides[cleanTitleLower]) {
            ...
            // SEC-11 FIX: Validate override value is a valid IMDB ID format
            if (/^tt\d+$/.test(imdbId)) {
