 ...
 * ALWAYS uses Authorization header — API key is NEVER in the URL.
 * TMDB v4 bearer tokens and v3 API keys are both sent as Bearer tokens.
 * TMDB's v3 endpoint accepts api_key as both query param AND Authorization header.
...
/**
 * SEC-05 FIX: No longer appends API key to URL.
 * Key is always sent via Authorization header (getTmdbHeaders).
 * Kept as a no-op for backward compatibility with callers.
 * @param {string} url
 * @param {string} _apiKey - Unused (kept for API compatibility)
 * @returns {string} The URL unchanged
 */
function appendApiKey(url, _apiKey) {
    // SEC-05 FIX: Never append API key to URL query string.
    // The key is always sent in the Authorization header.
    return url;
}
...
async function _matchTMDBInternal({ item, type, apiKey, overrides, title, year, cacheKey }) {
    try {
        ...
        if (year) {
            finalSearchUrl += type === "tv" ? `&first_air_date_year=${year}` : `&primary_release_year=${year}`;
        }

        const sRes = await fetchWithTimeout(finalSearchUrl, { headers }, DEFAULTS.TMDB_TIMEOUT);
        if (!sRes.ok) {
...
async function validateTmdbKey(apiKey) {
    ...
    try {
        ...
        const r = await fetchWithTimeout(url, { headers }, DEFAULTS.TMDB_TIMEOUT);
