 ...
 * Accepts both v3 API keys (32 hex) and v4 Read Access Tokens (JWT, may contain dots).
 * @param {string} key
 * @returns {boolean}
 */
function isValidApiKeyFormat(key) {
    ...
    // TMDB v3 keys: 32 hex chars; v4 bearer tokens: longer, may contain dots
    return trimmed.length >= 20 && trimmed.length <= 500 && /^[a-zA-Z0-9._\-]+$/.test(trimmed);
}
