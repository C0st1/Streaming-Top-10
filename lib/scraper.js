...
async function fetchFlixPatrolTitles(categoryType, country = "Global") {
    ...
    try {
        ...
        // ============================================================
        // NEW: FlixPatrol 2025+ HTML structure uses section IDs
        // - id="toc-netflix-tv-shows" for TV Shows section
        // - id="toc-netflix-movies" for Movies section
        // Headers are now: "TOP TV Shows on Netflix..." and "TOP Movies on Netflix..."
        // ============================================================
        
        // Determine which section ID to use based on category type
        const sectionId = categoryType === "Films" 
            ? 'toc-netflix-movies' 
            : 'toc-netflix-tv-shows';

        // Strategy 1: Find section by ID (new FlixPatrol structure)
        ...
        // Strategy 2: Fallback - Find by header text (legacy/alternate format)
        if (titles.length === 0) {
            ...
            const headers = $(`h2:contains("${altHeader}"), h3:contains("${altHeader}"), :contains("${targetHeader}")`).filter(function () {
                const text = $(this).text().trim();
                return text.includes(altHeader) || text.includes(targetHeader);
            });

            if (headers.length > 0) {
                console.log(`[Scraper] Found header "${altHeader}" for ${country}/${categoryType}`);
                let container = headers.first().parent().parent();
                if (container.length === 0) container = headers.first().closest('div[id*="toc-"]');
                if (container.length === 0) container = headers.first().closest('.content, .card, .table-wrapper');
        ...
        // Strategy 3: Final fallback — collect all title links from the page
        if (titles.length === 0) {
